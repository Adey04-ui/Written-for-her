 ...
 * 4. Hugging Face Inference API (free, no key)
 * 5. Advanced Combinatorial NLG (local, no network)
 */
...
${eventContext}
Details:
- How we met: ${met || 'a special place'}
...
Write 3-4 heartfelt paragraphs. Make it deeply personal using the specific details. Do not use generic placeholders. End warmly and emotionally.`;
}
...
// ============================================================
// MAIN GENERATOR — Tries all LLMs in order, falls back to NLG
// ============================================================

async function generateLetter(data) {
  const prompt = buildPrompt(data);

  // Priority order: OpenAI → Anthropic → Gemini → HuggingFace → NLG
  const backends = [
